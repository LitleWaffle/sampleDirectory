...
void hyperlink_to_event_tagid(int tagid){
  ...
  zEventId = db_text(0, "SELECT substr(tagname, 7) FROM tag WHERE tagid=%d",
                     tagid);
  cgi_printf("[%z%S</a>]\n",(href("%R/event/%s",zEventId)),(zEventId));
  free(zEventId);
}
...
void event_page(void){
  ...
  */
  ...
  if( rid==0 || (specRid!=0 && specRid!=rid) ){
    ...
    cgi_printf("Cannot locate specified event\n");
    style_footer();
    return;
  }
  ...
  */
  ...
  if( verboseFlag && g.perm.Hyperlink ){
    ...
    cgi_printf("<p>Event [%z%S</a>] at\n"
           "[%z%s</a>]\n"
           "entered by user <b>%h</b> on\n"
           "[%z%s</a>]:</p>\n"
           "<blockquote>\n",(href("%R/artifact/%s",zUuid)),(zUuid),(href("%R/timeline?c=%T",zETime)),(zETime),(pEvent->zUser),(href("%R/timeline?c=%T",zATime)),(zATime));
  ...
  */
  ...
  if( P("preview")!=0 ){
    ...
    if( wiki_find_title(&event, &title, &tail) ){
      cgi_printf("<h2 align=\"center\">%h</h2>\n",(blob_str(&title)));
      wiki_convert(&tail, 0, 0);
    }else{
  ...
  if( n<20 ) n = 20;
  if( n>40 ) n = 40;
  cgi_printf("<form method=\"post\" action=\"%s/eventedit\"><div>\n",(g.zTop));
  login_insert_csrf_secret();
  cgi_printf("<input type=\"hidden\" name=\"name\" value=\"%h\" />\n"
         "<table border=\"0\" cellspacing=\"10\">\n",(zEventId));

  cgi_printf("<tr><th align=\"right\" valign=\"top\">Event&nbsp;Time (UTC):</th>\n"
         "<td valign=\"top\">\n"
