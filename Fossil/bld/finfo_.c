...
void finfo_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( zUuid ){
      ...
      cgi_printf("%z[%S]</a> part of check-in\n",(href("%R/artifact/%s",zUuid)),(zUuid));
    }else{
      char *zNewName;
      zNewName = db_text(0,
    ...
    if( g.perm.Hyperlink && zUuid ){
      ...
      cgi_printf("%z\n"
             ...
             "%z\n"
             "[blame]</a>\n"
             "%z[checkins&nbsp;using]</a>\n",(href("%R/annotate?checkin=%S&filename=%h",zCkin,z)),(href("%R/blame?checkin=%S&filename=%h",zCkin,z)),(href("%R/timeline?n=200&uf=%S",zUuid)));
      if( fpid ){
        cgi_printf("%z[diff]</a>\n",(href("%R/fdiff?v1=%S&v2=%S&sbs=1",zPUuid,zUuid)));
      }
    }
    if( fDebug & FINFO_DEBUG_MLINK ){
      ...
      cgi_printf("<br>fid=%d pid=%d mid=%d sz=%d\n",(frid),(fpid),(fmid),(sz));
      if( srcid ){
        cgi_printf("srcid=%d\n",(srcid));
      }
    }
