...
void attachlist_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<br><a href=\"/attachview?%s\">%h</a>\n"
           "[<a href=\"/attachdownload/%t?%s\">download</a>]<br />\n",(zUrlTail),(zFilename),(zFilename),(zUrlTail));
    ...
    if( zPage==0 && zTkt==0 ){
      ...
      if( strlen(zTarget)==UUID_SIZE && validate16(zTarget, UUID_SIZE) ){
        cgi_printf("%s ticket <a href=\"%s/tktview?name=%s\">\n"
               "%S</a>\n",(zSrc),(g.zTop),(zTarget),(zTarget));
      }else{
        cgi_printf("%s wiki page <a href=\"%s/wiki?name=%t\">\n"
               "%h</a>\n",(zSrc),(g.zTop),(zTarget),(zTarget));
      }
    }else{
      if( zSrc==0 || zSrc[0]==0 ){
