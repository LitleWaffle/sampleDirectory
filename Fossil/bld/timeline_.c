...
void hyperlink_to_uuid(const char *zUuid){
  ...
  if( g.perm.Hyperlink ){
    cgi_printf("%z[%s]</a>\n",(xhref("class='timelineHistLink'","%R/info/%s",z)),(z));
  }else{
...
void hyperlink_to_diff(const char *zV1, const char *zV2){
  if( g.perm.Hyperlink ){
    if( zV2==0 ){
      cgi_printf("%z[diff]</a>\n",(href("%R/diff?v2=%s",zV1)));
    }else{
      cgi_printf("%z[diff]</a>\n",(href("%R/diff?v1=%s&v2=%s",zV1,zV2)));
    }
  }
}
      ...
      while( db_step(&fchngQuery)==SQLITE_ROW ){
        ...
        if( (tmFlags & TIMELINE_FRENAMES)!=0 ){
          if( !isNew && !isDel && zOldName!=0 ){
            cgi_printf("<li> %h &rarr; %h\n",(zOldName),(zFilename));
          }
          continue;
        }
        if( isNew ){
          cgi_printf("<li> %h (new file) &nbsp;\n"
                 "%z[view]</a></li>\n",(zFilename),(href("%R/artifact/%S",zNew)));
        }else if( isDel ){
          cgi_printf("<li> %h (deleted)</li>\n",(zFilename));
        }else if( fossil_strcmp(zOld,zNew)==0 && zOldName!=0 ){
          cgi_printf("<li> %h &rarr; %h\n"
                 "%z[view]</a></li>\n",(zOldName),(zFilename),(href("%R/artifact/%S",zNew)));
        }else{
          if( zOldName!=0 ){
            cgi_printf("<li> %h &rarr; %h\n",(zOldName),(zFilename));
          }else{
...
void timeline_output_graph_javascript(
  ...
  if( pGraph && pGraph->nErr==0 && pGraph->nRow>0 ){
    ...
    */
    ...
    graph_free(pGraph);
    cgi_printf("var canvasDiv = gebi(\"canvas\");\n"
...
void test_timewarp_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<li>\n"
           "<a href=\"%s/timeline?p=%S&amp;d=%S&amp;unhide\">%S</a>\n",(g.zTop),(zUuid),(zUuid),(zUuid));
  }
  db_finalize(&q);
  style_footer();
}
...
static void stats_report_by_user(){
  ...
  while( SQLITE_ROW == db_step(&query) ){
   ...
   cgi_printf("<tr class='row%d'>\n"
           ...
           " style='height:16px;width:%d%%;'>\n"
           "</div></td>\n"
          "</tr>\n",(rowClass),(zUser),((char)statsReportType),(zUser),(nCount),(nSize));
    /*
      Potential improvement: calculate the min/max event counts and
      use percent-based graph bars.
    */
...
void stats_report_page(){
  ...
  if(0==fossil_strcmp(zView,"byyear")){
    ...
    cgi_printf("<h1>Select a report to show:</h1>\n"
           "<ul>\n"
